...
(function($, _) {
    ...
    AddressSelect.prototype = {
        build: function() {
            ...
            return this.$element.each(function() {
                ...
                AddressSelect.buildSelect($countrySelect, 1, 0);

                if (o.country_val != undefined) {
                    $countrySelect.val(o.country_val);
                    AddressSelect.buildSelect($provinceSelect, 2, o.country_val);
                } else {
                    AddressSelect.emptySelect($provinceSelect, 2);
                }

                if (o.province_val != undefined) {
                    $provinceSelect.val(o.province_val);
                    AddressSelect.buildSelect($citySelect, 3, o.province_val);
                } else {
                    AddressSelect.emptySelect($citySelect, 3);
                }

                if (o.city_val != undefined) {
                    $citySelect.val(o.city_val);
                    AddressSelect.buildSelect($districtSelect, 4, o.city_val);
                } else {
                    AddressSelect.emptySelect($districtSelect, 4);
                }
                ...
                AddressSelect.bindChange($countrySelect, o.callback, $provinceSelect, 2);
                AddressSelect.bindChange($provinceSelect, o.callback, $citySelect, 3);
                AddressSelect.bindChange($citySelect, o.callback, $districtSelect, 4);
                AddressSelect.bindChange($districtSelect, o.callback);
            });
        }
    };

    AddressSelect.bindChange = function($select, callback, $subSelect, type) {
        $select.change(function() {
            var val = parseInt($(this).val());
            if($subSelect) {
                AddressSelect.buildSelect($subSelect, type, val);
            }
    ...
    AddressSelect.emptySelect = function($select, type) {
        $select.empty();
        $select.append('<option value="0">' + emptyAddress[type - 1] + '</option>');
    };

    AddressSelect.buildSelect = function($select, type, pid, defVal) {
